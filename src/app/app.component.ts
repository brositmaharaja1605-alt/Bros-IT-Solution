...
interface Stat {
  ...
  selector: 'app-root',
  standalone: true,
  imports: [CommonModule, RouterOutlet, NavbarComponent, AboutComponent, WhatsappPopupComponent ],
  templateUrl: './app.component.html',
})

  // All styling lives in the global src/styles.scss (see comment there for why:
  // the theme toggle sets data-theme on <html>, which needs a global stylesheet,
  // not a view-encapsulated component stylesheet).
...
window.scrollTo({top:0,behavior:'smooth'});
  ...
  ngOnInit(): void {
    this.faqCats = Object.keys(this.faqData);
  ...
  /* ---------- TESTIMONIALS ---------- */
  get currentTesti(): Testimonial {
    return this.testimonials[this.testiIndex];
  }
  prevTesti(): void {
    this.testiIndex = (this.testiIndex - 1 + this.testimonials.length) % this.testimonials.length;
  }
  nextTesti(): void {
    this.testiIndex = (this.testiIndex + 1) % this.testimonials.length;
  }
  goToTesti(i: number): void {
    this.testiIndex = i;
  }

  /* ---------- FAQ ---------- */
  setFaqCat(c: string): void {
    this.activeCat = c;
    this.openFaqIndex = 0;
  }
